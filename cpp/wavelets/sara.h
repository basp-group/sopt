...
class SARA : public std::vector<Wavelet>
{
  public:
    ...
    template<class T0>
      auto direct(Eigen::ArrayBase<T0> const &signal) const -> decltype(copy(signal));
    //! \brief Direct transform
    ...
    //! \details Supports 1 and 2 dimensional tranforms for real and complex data.
    template<class T0>
      auto indirect(Eigen::ArrayBase<T0> const &coeffs) const -> decltype(copy(coeffs));
    //! \brief Indirect transform
    //! \param[in] coefficients: Input wavelet coefficients. Its size must be a multiple of $2^l$
    //! where $l$ is the number of levels. Can be a matrix (2d-transform) or a column vector (1-d
...
template<class T0>
  auto SARA::indirect(Eigen::ArrayBase<T0> const &coeffs) const -> decltype(copy(coeffs)) {
...
template<class T0>
  auto SARA::direct(Eigen::ArrayBase<T0> const &signal) const -> decltype(copy(signal)) {
